...
class HeartbeatQueueProcessor(threading.Thread):
    ...
    def send(cls, heartbeat, extra_heartbeats=None):
        ...
        for pattern in SETTINGS.get(
            settings, "ignore", fallback=[]
        ):  # or should it be blender-specific?
...
def enough_time_passed(now, is_write):
    return _last_hb is None or (
        now - _last_hb["timestamp"] > (2 if is_write else HEARTBEAT_FREQUENCY * 60)
    )


@lru_cache(maxsize=128)
def guessProjectName(
    filename, truncate_trail, use_project_folder, project_prefix, project_postfix
):
...
def handle_activity(is_write=False):
    ...
    if _filename and (
        _filename != last_file or enough_time_passed(timestamp, is_write)
    ):
        blender_settings = bpy.context.preferences.addons[__name__].preferences
